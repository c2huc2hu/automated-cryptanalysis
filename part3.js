...
function vigenere_crack_sa(cipher, model, key_length, iters=50000) {
    ...
    let initial_key = _.times(key_length, x => _.random(26-1));
...
function substitution_crack_sa(cipher, model, iters=50000) {
    ...
    var initial_key = _.shuffle('ABCDEFGHIJKLMNOPQRSTUVWXYZ').join('');
    ...
    let result_key = anneal(prob_fcn, transition, initial_key, iters);
    return {key: result_key, message: part1.poly_decrypt(cipher, result_key), score: prob_fcn(result_key)};
}
...
function anneal(prob_fcn, transition, initial_state, iters=100000) {
    ...
    for(let i=0; i<iters + iters / 1; i++) {
        ...
        // if(i % Math.floor(iters / 20) == 0) {
            // console.log(candidate_state, candidate_cost, curr_state, curr_cost);
...
var sentence = part1.poly_encrypt('a form of asbestos once used to make kent cigarette filters has caused a high percentage of cancer deaths'.toUpperCase(), 'qwertyuiopasdfghjklzxcvbnm'.toUpperCase());
console.log("decrypting with substitution:", sentence);
var t = Date.now();
for(let i=0; i<10; i++) {
    console.log(substitution_crack_sa(sentence, model, iters=150000));
    console.log('Took', (Date.now() - t) / 1000, 'seconds');
}